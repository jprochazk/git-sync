...
export default async function (pos: string[], flags: Record<string, any>) {
  ...
  if (hasChanges && !flags.stash) {
    await $`git stash pop`;
    $.fail(
      `You have unsaved changes, please stash/commit them, or run the command again with the \`--stash\` flag.`
    );
  }
  ...
  if (status.includes("up to date")) {
    ...
    if (status.includes("git rebase --continue")) {
      ...
      let conflicts = [...status.matchAll(/Merge conflict in (.+)/g)].map(
        (e) => e[1]
      );
      while (true) {
        console.log(
          `Please resolve conflicts in: ${conflicts
            .map((t) => green(t))
            .join(" ")}`
        );
        const key = await $.key;
        ...
        const status =
          $.os.platform() === "win32"
            ? await $`cmd /V /C "set GIT_EDITOR=true && git rebase --continue"`
                .silent
        ...
        conflicts = [...status.matchAll(/Merge conflict in (.+)/g)].map(
          (e) => e[1]
        );
      }
    }
