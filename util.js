...
function capture(/** @type {string} */ cmd, /** @type {{silent: boolean, env?: Record<string, any>}} */ ctx) {
  return new Promise(resolve => setTimeout(() => {
    ...
    !ctx.silent && console.log(`$ ${Object.entries(ctx.env ?? {}).map(([k,v]) => `${k}="${v}"`).join(" ")} ${cmd}`);
  }, 0))
}

/**
 * @typedef PromiseExtra
 * @property {Promise<string> & PromiseExtra} silent
 */
/**
 * @returns {Promise<string> & PromiseExtra}
 */
...
$.remote = Promise.resolve("");
{
  ...
  Object.defineProperty($, "remote", {
    get() {
      return new Promise(async resolve => resolve(await $`git remote show`.silent))
    }
  });
}
...
$.dedent = (/** @type {TemplateStringsArray} */ strings, /** @type {any[]} */ ...values) => {
...
const createColorFn = (/** @type {string} */ color) => (/** @type {string} */ text) => `${color}${text}${colors.Reset}`;
